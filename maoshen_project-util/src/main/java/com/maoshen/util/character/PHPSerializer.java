...
public class PHPSerializer {
	...
	public static String getJson(Object obj) {
		String str = null;
		try {
			String tmp = new String(serialize(obj), "UTF-8");
			if (!StringUtil.isNullOrBlank(tmp)) {
				int startKey = tmp.indexOf("\"");
				int endKey = tmp.lastIndexOf("\"");
				if (-1 != startKey && -1 != endKey && startKey < endKey) {
					str = tmp.substring(startKey + 1, endKey);
				}
			}
	...
	public static int serialize(ByteArrayOutputStream stream, Object obj,
			HashMap<Integer, Integer> ht, int hv, String charset) {
		if (obj == null) {
			hv++;
			writeNull(stream);
		} else {
